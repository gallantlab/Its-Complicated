...
`IMRIAgentComponentVisitor` is a pure virtual interface with the `Visit()` method, one per concrete agent type. `MRIExperimentState` implements this interface: when it visits an agent, it creates the appripriate Entity State. This lets the experiment state record every tracked entity's position, rotation, type, and ID without the state class needing to know about specific actor classes.

## How to Use

Fro each actor class you want tracked during the experiment, create a matching `AgentComponent` that is attached to the class that tracks the relevant state information.
